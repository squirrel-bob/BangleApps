...
* `appid.info` is JSON that describes an app - this is auto-generated by the App Loader
* `appid.img` is an image
* `appid.app.js` is JS code for applications
* `appid.wid.js` is JS code for [widgets](https://www.espruino.com/Bangle.js+Widgets)
* `appid.clkinfo.js` is JS code for a [clock info](https://www.espruino.com/Bangle.js+Clock+Info)
* `appid.settings.js` is JS code for [the settings menu](https://www.espruino.com/Bangle.js+App+Settings)
* `appid.boot.js` is JS code that automatically gets run at boot time
* `appid.json` is used for JSON settings for an app
